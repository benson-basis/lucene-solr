...
public class DirectPostingDirectoryReader extends FilterDirectoryReader {
  ...
  static class DPFAtomicFilterReader extends FilterAtomicReader {
    ...
    public Fields fields() throws IOException {
      return new DirectFields(in.getFieldInfos(), in.fields(), minSkipCount, lowFreqCutoff);
    }
  }
  ...
  final static class DirectField extends Terms {
    ...
    public DirectField(FieldInfos fieldInfos, String field, Terms termsIn, int minSkipCount, int lowFreqCutoff) throws IOException {
      ...
      hasFreq = fieldInfo.getIndexOptions().compareTo(FieldInfo.IndexOptions.DOCS_ONLY) > 0;
      hasPos = fieldInfo.getIndexOptions().compareTo(FieldInfo.IndexOptions.DOCS_AND_FREQS) > 0;
      hasOffsets = fieldInfo.getIndexOptions().compareTo(FieldInfo.IndexOptions.DOCS_AND_FREQS_AND_POSITIONS) > 0;
